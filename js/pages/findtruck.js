...
function renderFindTruck() {
    const availableTrucks = DB.getCachedTrucks().filter(t => t.status === 'Available');
    return `
...
function filterTrucks() {
    ...
    let trucks = DB.getCachedTrucks().filter(t => t.status === 'Available');
    ...
    document.getElementById('truckResults').innerHTML = trucks.length ?
        ...
        '<div class="empty-state"><h3>No trucks found</h3><p>Try adjusting filters</p></div>';
