...
function renderMarketplace() {
    ...
    <div class="animate-fadeIn">
        ...
        <div class="tabs" id="marketplaceTabs">
            ...
            <button class="tab-btn" onclick="switchMarketTab('matched')">✅ Matched (${MockData.loads.filter(l=>l.status==='Matched').length})</button>
        </div>
...
function switchMarketTab(tab) {
    ...
    if (tab === 'loads') {
        const loads = MockData.loads.filter(l => l.status === 'Pending');
        content.innerHTML = loads.slice(0, 12).map(l => UI.loadCard(l)).join('');
    } else if (tab === 'trucks') {
        const trucks = MockData.trucks.filter(t => t.status === 'Available');
        content.innerHTML = trucks.slice(0, 12).map(t => UI.truckCard(t)).join('');
    } else {
        const matched = MockData.loads.filter(l => l.status === 'Matched');
...
function filterMarket(type, el) {
    ...
    const content = document.getElementById('marketplaceContent');
    let loads = MockData.loads.filter(l => l.status === 'Pending');
